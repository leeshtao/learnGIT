	...
	*/
	private void merge(int[] array, int left, int mid, int right) {
		for (int k = left; k <= right; k++) {
			// 将区间[left,right]复制到temp数组中，这是强硬合并，并没有温柔的捋顺。
			temp[k] = array[k];
		}
		int i = left;
		int j = mid + 1;
		for (int k = left; k <= right; k++) {
			// 通过判断，将辅助数组temp中的值按照大小归并回原数组array
			if (i > mid)
				// 第三步：左半边用尽，则取右半边元素
				array[k] = temp[j++];// 右侧元素取出一个以后，要移动指针到其右侧下一个元素了。
			else if (j > right)// 第四步：与第三步同步，右半边用尽，则取左半边元素
				array[k] = temp[i++];// 同样的，左侧元素取出一个以后，要移动指针到其右侧下一个元素了。
			else if (array[j] > temp[i])// 第一步：右半边当前元素大于左半边当前元素，取右半边元素（从大到小排序）
				array[k] = temp[j++];// 右侧元素取出一个以后，要移动指针到其右侧下一个元素了。
			else// 第二步：与第一步同步，左半边当前元素大于右半边当前元素，取左半边元素（从大到小排序）
				array[k] = temp[i++];// 同样的，左侧元素取出一个以后，要移动指针到其右侧下一个元素了。
		}
	}
